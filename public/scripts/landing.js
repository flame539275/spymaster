...
class Landing extends React.Component {
  ...
  constructor(props) {
    ...
    txtFile = "../../wordlist.txt"          //maybe filepath needs fixing
    ...
    file.close()
    do{
      this.state.proposedID = wordArray[Math.floor(Math.random() * wordArray.length)]
      $.post({
        contentType: 'application/json; charset=utf-8',
        dataType: 'json',
        data: JSON.stringify({ game_id: proposedID }),
    ...
    }while(this.state.invalidID)
    this.setState({ gameId: this.state.proposedID})  
